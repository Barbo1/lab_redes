...
class Server(object):
    ...
    def get_json_rpc_error(self, jsonrpc, code):
        # tomar el mensaje de salida en base al codigo
        if code == -32700:
            message = "Parse error"
        elif code == -32600:
        ...
        # retorno del jsonrpc de salida.
        return {"jsonrpc": jsonrpc, "error": {"code": code, "message": message}, "id": "null"}

    def validate_json_rpc(self, json):
        if type(json) is dict:
            if "jsonrpc" in json and json["jsonrpc"] == self.json_rpc_version and "method" in json and type(json["method"]) is str:
                args = list(json.keys())
                args.remove("jsonrpc")
                args.remove("method")
                if len(args) == 2:
